...
from utils import augment_EEG, cart2sph, pol2cart, reformatInput
...
def CV_indices(data, labels, kfold):
    '''
    Splits the input data into train and validation sets for each fold of cross validation.
    :param data: 4 or 5 dimensional array of data
    :param labels: true labels
    :param kfold: object of sklearn.model_selection.Kfold class
    :return: generator object, generator of pairs (train_indices, val_indices) for one fold per iteration
    '''
    if data.ndim == 4:
        return kfold.split(data)
    elif data.ndim == 5:
        return kfold.split(np.transpose(data, axes=[1,0,2,3,4]))
...
def build_convpool_conv1d(input_vars, nb_classes, imsize=32, n_colors=3, n_timewin=3):
    ...
    convpool = Conv1DLayer(convpool, 19, 3)
    # A fully-connected layer of 512 units with 50% dropout on its inputs:
    convpool = DenseLayer(lasagne.layers.dropout(convpool, p=.5),
            num_units=512, nonlinearity=lasagne.nonlinearities.rectify)
    # And, finally, the output layer with 50% dropout on its inputs:
...
def build_convpool_mix(input_vars, nb_classes, grad_clip=110, imsize=32, n_colors=3, n_timewin=3, dropout=.5):
    """
    ...
    convnets = []
    w_init = None
    # Build <number of timewindows> parallel CNNs with shared weights
    for i in range(n_timewin):
        if i == 0:
            convnet, w_init = build_cnn(input_vars[i], imsize=imsize, n_colors=n_colors)
        else:
    ...
    # input to 1D convlayer should be in (batch_size, num_input_channels, input_length)
    conv_out = Conv1DLayer(reformConvpool, 19, 3)
    ...
    convpool = DenseLayer(lasagne.layers.dropout(dense_input, p=dropout),
            num_units=512, nonlinearity=lasagne.nonlinearities.rectify)
...
def train(images, labels, fold, model_type, batch_size=32, num_epochs=5):
    ...
    # As a bonus, also create an expression for the classification accuracy and auc:
    ...
    val_fn = theano.function([input_var, target_var], [test_loss, test_acc, test_prediction]) # My supplement (test_prediction)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_val, y_val, batch_size, shuffle=False):
            inputs, targets = batch
            err, acc, _ = val_fn(inputs, targets) # My supplement (auc)
            val_err += err
            val_acc += acc
            val_batches += 1
        ...
        for batch in iterate_minibatches(X_test, y_test, batch_size, shuffle=False):
            inputs, targets = batch
            err, acc, y_p = val_fn(inputs, targets)
            test_err += err
            test_acc += acc
            test_batches += 1
        ...
        ############
        err, acc, y_pred = val_fn(X_test, y_test)
    ...
    print("Best test auc:\t\t{:.5f}".format(best_test_auc))

    frp, trp, _ = roc_curve(y_test, y_pred[:,1])
    plt.plot(frp,trp)
    plt.xlabel('FRP')
    plt.ylabel('TRP')
    plt.show()

def train_save(images, labels, fold, model_type, dirname,batch_size=32,
               num_epochs=5, learning_rate=.001, l1_coef=.0, l2_coef=.0, dropout=.5, n_timewin=4):
    """
    A sample training function which loops over the training set and evaluates the network
    on the validation set after each epoch. Evaluates the network on the training set
    whenever the
    ...
    :param fold: tuple of (train, test) index numbers
    :param model_type: model type ('cnn', '1dconv', 'maxpool', 'lstm', 'mix')
    :param filename: path to the directory, where to put results
    :param batch_size: batch size for training
    :param num_epochs: number of epochs of dataset to go over for training
    :param learning_rate: learning rate
    :param l1_coef: weight of l1 norm in loss function (l1 regularization). Only in cnn and mix models.
    :param l2_coef: weight of l2 norm in loss function (l2 regularization). Only in cnn and mix models.
    :param dropout: The dropout rate, between 0 and 1. E.g. "rate=0.1" would drop out 10% of input units.
    :return: none
    """
    num_classes = len(np.unique(labels))
    (X_train, y_train), (X_val, y_val) = reformatInput(images, labels, fold, test=False)
    X_train = X_train.astype("float32", casting='unsafe')
    X_val = X_val.astype("float32", casting='unsafe')
    # Prepare Theano variables for inputs and targets
    input_var = T.TensorType('floatX', ((False,) * 5))()
    ...
    if model_type == '1dconv':
        ...
        network = build_convpool_lstm(input_var, num_classes, 100, n_timewin=4)
    elif model_type == 'mix':
        network = build_convpool_mix(input_var, num_classes, 100, dropout=dropout)
    elif model_type == 'cnn':
        input_var = T.tensor4('inputs')
        network, _ = build_cnn(input_var)
        network = DenseLayer(lasagne.layers.dropout(network, p=dropout),
                             num_units=256,
                             nonlinearity=lasagne.nonlinearities.rectify)
        network = DenseLayer(lasagne.layers.dropout(network, p=dropout),
                             num_units=num_classes,
    ...
    prediction = lasagne.layers.get_output(network)
    loss = lasagne.objectives.categorical_crossentropy(prediction, target_var)
    loss = loss.mean() + l1_coef * regularize_network_params(network, l1) +\
           l2_coef * regularize_network_params(network, l2)
    ...
    test_loss = lasagne.objectives.categorical_crossentropy(test_prediction,
                                                            target_var)
    test_loss = test_loss.mean() + l1_coef * regularize_network_params(network, l1) +\
           l2_coef * regularize_network_params(network, l2)
    # As a bonus, also create an expression for the classification accuracy and auc:
    test_acc = T.mean(T.eq(T.argmax(test_prediction, axis=1), target_var),
                      dtype=theano.config.floatX)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in iterate_minibatches(X_val, y_val, batch_size, shuffle=False):
            inputs, targets = batch
            err, acc, y_pred = val_fn(inputs, targets)
            val_err += err
            val_acc += acc
            val_batches += 1
        av_train_err = train_err / train_batches
        ...
        err, acc, y_pred = val_fn(X_val, y_val)
        auc = roc_auc_score(y_val, y_pred[:, 1])

        train_loss = np.append(train_loss, av_train_err)
        val_loss = np.append(val_loss, av_val_err)
        val_accu = np.append(val_accu, av_val_acc * 100)
        val_accu_another = np.append(val_accu_another, acc)
        val_auc = np.append(val_auc, auc)

        if auc > best_val_auc:
            best_val_auc = auc
            # Checkpoint
            np.savez(dirname + 'weights_{0}_lr{1}_l1{2}_l2{3}_dr{4}'.format(model_type, learning_rate, l1_coef, l2_coef, dropout),
    ...
    uniq = model_type + '_ep_' + str(num_epochs) + '_lr_' + str(learning_rate) +\
           '_l1_' + str(l1_coef) + '_l2_' + str(l2_coef) + '_dr_' + str(dropout)

    saveResults(dirname, train_loss, val_loss, val_accu, val_auc, uniq=uniq)
    plotResults(dirname, train_loss, val_loss, val_accu, val_auc, num_epochs,  val_accu_another, uniq=uniq,)

def train_test(X_train, X_test,y_train, y_test, model_type, dirname, filename, batch_size=32,
               num_epochs=5, learning_rate=.001, l1_coef=.0, l2_coef=.0, dropout=.5):
    """
    A sample training function which loops over the training set and evaluates the network
    on the validation set after each epoch. Evaluates the network on the training set
    whenever the
    :param X_train: ndarray input images for training
    :param X_test: ndarray input images for testing or validation
    :param y_train: 1D array train labels
    :param y_test: 1D array test labels
    :param labels: target labels
    :param fold: tuple of (train, test) index numbers
    :param model_type: model type ('cnn', '1dconv', 'maxpool', 'lstm', 'mix')
    :param dirname: str name of directory where to put results
    :param filename: file name where to write scores (roc auc)
    :param batch_size: batch size for training
    :param num_epochs: number of epochs of dataset to go over for training
    :param learning_rate: learning rate
    :param l1_coef: weight of l1 norm in loss function (l1 regularization). Only in cnn and mix models.
    ...
    for epoch in range(num_epochs):
        ...
        if auc > best_val_auc:
            best_val_auc = auc
            # Checkpoint
            np.savez(os.path.join(dirname, 'weights_{0}_lr{1}_l1{2}_l2{3}_dr{4}'.format(model_type, learning_rate, l1_coef, l2_coef, dropout)),
                     *lasagne.layers.get_all_param_values(network))
...
def loadlocs(path):
    ...
    return elNum, locs_2d

def saveResults(dirname, train_loss, val_loss, val_accu, auc, uniq=''):

    fileTL = open(os.path.join(dirname, uniq + '_tr_loss.npy'), 'w')
    np.save(fileTL, train_loss)
    fileTL.close()

    fileVL = open(os.path.join(dirname, uniq + '_val_loss.npy'), 'w')
    np.save(fileVL, val_loss)
    fileVL.close()

    fileVA = open(os.path.join(dirname, uniq + '_val_acc.npy'), 'w')
    np.save(fileVA, val_accu)
    fileVA.close()

    fileA = open(os.path.join(dirname, uniq + '_auc.npy'), 'w')
    np.save(fileA, auc)
    fileA.close()


def plotResults(dirname, train_loss, val_loss, val_accu, auc, num_epochs,  val_accu_another, uniq):
    ...
    plt.savefig(os.path.join(dirname, uniq + '_val_auc.png'))
    ...
    plt.savefig(os.path.join(dirname, uniq + '_val_acc.png'))

    plt.cla()
    plt.clf()

    # Plot another accuracy
    plt.plot(epochs, val_accu_another)
    plt.xlabel('epochs')
    plt.ylabel('%')
    plt.title('Validation accuracy')
    plt.savefig(os.path.join(dirname, uniq + '_val_acc_another.png'))
    ...
    plt.savefig(os.path.join(dirname, uniq + '_loss.png'))
...
if __name__ == '__main__':
    ...
    feats = scipy.io.loadmat('C:/Users/alena/Desktop/homed/laba/normData/FeatureMat_timeWin.mat')['features']
    subj_nums = np.squeeze(scipy.io.loadmat('C:/Users/alena/Desktop/homed/laba/normData/trials_subNums.mat')['subjectNum'])
    # Leave-Subject-Out cross validation
    fold_pairs = [] # Pairs of test and train data for validation
    for i in np.unique(subj_nums):
        ts = subj_nums == i
        tr = np.squeeze(np.nonzero(np.bitwise_not(ts))) # indices of trials corresponding to all  subjects except for i-th
    ...
    av_feats = reduce(lambda x, y: x+y, [feats[:, i*3*elNum:(i+1)*3*elNum] for i in range(feats.shape[1] / (3*elNum))])
    ...
    train(images, np.squeeze(feats[:, -1]) - 1, fold_pairs[2], 'cnn', num_epochs=10)
    ...
    train(images_timewin, np.squeeze(feats[:, -1]) - 1, fold_pairs[2], 'mix',num_epochs=10)
