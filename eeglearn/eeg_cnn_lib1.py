...
def train_without_test(images, labels, fold, model_type, dir_name, learning_rate=0.001, batch_size=32, num_epochs=5):
    """
    A sample training function which loops over the training set and evaluates the network
    on the validation set after each epoch (but without testing). After that it makes plots of
    train and validation accuracy depending on epochs number, validation ROC and compute AUC on the validation dataset.
    Evaluates the network on the training set whenever the
    :param images: input images
    :param labels: target labels
    :param fold: tuple of (train, test) index numbers
    :param model_type: model type ('cnn', '1dconv', 'maxpool', 'lstm', 'mix')
    :param learning_rate: learning rate using in adam optimizer
    :param dir_name: path to directory where to save plots of
    :param batch_size: batch size for training
    :param num_epochs: number of epochs of dataset to go over for training
    :return: none
    """
    ...
    print('-'*50)
    print("Best validation accuracy:\t\t{:.2f} %".format(best_validation_accu * 100))
    print("Best validation auc:\t\t{:.5f}".format(best_val_auc))

    epochs = np.arange(1, num_epochs + 1)
    # Plot ROC for the last validation results
    frp, trp, _ = roc_curve(y_val, y_pred[:,1])
    plt.plot(frp,trp)
    plt.xlabel('FRP')
    plt.ylabel('TRP')
    plt.title('ROC in ' + model_type + ' model (validation after' + str(num_epochs) + ' epochs)')
    plt.savefig(dir_name + 'ROC_val_' + model_type + str(num_epochs) + '.png')
    plt.cla()
    plt.clf()
